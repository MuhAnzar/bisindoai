...
@section('judul', 'Edit Huruf')
@section('deskripsi', 'Perbarui informasi huruf dan video isyarat.')

@section('konten')
    <div style="display: flex; justify-content: center;">
        <div class="card" style="width: 100%; max-width: 1000px; padding: 32px;"> <!-- Widened card -->
            <form action="{{ route('admin.abjad.update', $abjad->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                
                <h3 style="margin-bottom: 24px; font-size: 1.25rem; border-bottom: 1px solid var(--border-color); padding-bottom: 12px;">Edit Huruf {{ $abjad->huruf }}</h3>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px; align-items: start;">
                    ...
                    <div>
                        ...
                        <div class="form-group" style="text-align: center; margin-bottom: 24px;">
                            <label for="huruf" style="display: block; margin-bottom: 12px; text-align: left;">Huruf Isyarat</label>
                            <input type="text" id="huruf" name="huruf" value="{{ old('huruf', $abjad->huruf) }}" maxlength="1" required 
                                style="width: 100px; height: 100px; text-transform: uppercase; font-size: 3rem; text-align: center; border: 2px solid var(--border-color); border-radius: 20px; font-weight: 700;">
                            @error('huruf') <div class="invalid-feedback" style="justify-content: center;">{{ $message }}</div> @enderror
                        </div>

                        <div class="form-group">
                            <label for="deskripsi">Deskripsi</label>
                            <textarea id="deskripsi" name="deskripsi" rows="6" style="resize: vertical; min-height: 120px;">{{ old('deskripsi', $abjad->deskripsi) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            @if($abjad->berkas_video)
                                <div style="margin-bottom: 16px; padding: 12px; border: 1px solid var(--border-color); border-radius: 12px; background: #f9fafb;">
                                    @php
                                        $extension = pathinfo($abjad->berkas_video, PATHINFO_EXTENSION);
                                        $isImage = in_array(strtolower($extension), ['jpg', 'jpeg', 'png', 'gif', 'webp']);
                                        $isVideo = in_array(strtolower($extension), ['mp4', 'webm', 'ogg', 'mov']);
                                    @endphp

                                    @if($isImage)
                                        <img src="{{ asset($abjad->berkas_video) }}" alt="Preview" style="max-width: 100%; max-height: 200px; border-radius: 8px; display: block; margin: 0 auto; object-fit: contain;">
                                    @elseif($isVideo)
                                        <video controls style="max-width: 100%; max-height: 200px; border-radius: 8px; display: block; margin: 0 auto;">
                                            <source src="{{ asset($abjad->berkas_video) }}" type="video/{{ $extension }}">
                                    ...
                                    @else
                                        <div style="display: flex; align-items: center; justify-content: space-between;">
                                            ...
                                            <a href="{{ asset($abjad->berkas_video) }}" target="_blank" class="text-sm" style="color: var(--primary); font-weight: 500; text-decoration: none;">Lihat File</a>
                                        </div>
                                    @endif
                                </div>
                            @else
